...
vim.keymap.set('n', '<leader>a', 'ggVG"+y', opts('全选并复制到剪切板'))
...
vim.keymap.set('n', '<leader>f', '<c-w>w', opts('切换窗口'))
...
vim.keymap.set('n', '-', function()
    -- 如果当前行在一个已关闭的折叠内（即它是折叠的首行且已关闭），则展开它
    if vim.fn.foldclosed('.') ~= -1 then
        return 'za'
    end
    -- 如果当前行是一个开启折叠的起始行，则折叠它
    local line = vim.fn.line('.')
    if vim.fn.foldlevel(line) > vim.fn.foldlevel(line - 1) then
        return 'za'
    end
    -- 否则，认为光标在对象内部但不在起始行，或者该处尚未创建折叠，尝试创建新折叠
    return 'va{zf^'
end, opts('折叠/展开', { expr = true }))
